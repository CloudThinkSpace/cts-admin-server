...
use models::dto::cts::request::form::SearchFormData;
...
pub async fn delete_by_id(table_id: String, id: String, force: bool) -> Result<String> {
    ...
    // 查询数据是否存在
    let table_name = get_table_name(&table_id, IS_DATA);
    let result = CtsSelect::table(&table_name).find_by_id(&id, true)
        .one(&db).await?;
    if result.is_some() {
        // 判断是否强制删除，如果是删除数据，如果不是更新删除字段
        let _ = CtsSelect::table(&table_name).delete_by_id(&id, force).execute(&db).await?;
        Ok(id)
    } else {
        bail!("无法删除，数据不存在")
    }
...
pub async fn update(table_id: String, id: String, data: Value) -> Result<String> {
    ...
    // 查询是否有数据
    let table_name = get_table_name(&table_id, IS_DATA);
    let result = CtsSelect::table(&table_name).find_by_id(&id, false).one(&db).await?;
    match result {
        None => {
            bail!("数据不存在，无法更新")
        }
        Some(_) => {
            let _ = CtsSelect::table(&table_name).update(&id, data)?.execute(&db).await?;
            Ok(id)
        }
    }
}
